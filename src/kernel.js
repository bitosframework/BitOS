 ...
 * BitOS Kernel
 *
 * @description Entrypoint of the BitOS kernel
...
export const BitOSTerminal = W3OSTerminal
...
/**
 * The primary Broadcast Channel for BitOS
 * 
 * @type {BroadcastChannel}
 * @see https://developer.mozilla.org/en-US/docs/Web/API/Broadcast_Channel_API
 */
export const broadcast = new BroadcastChannel('BitOS')
...
export async function printBootIntro () {
  ...
  log(colors.info(`${t('kernel:bootIntroSubtitle', '\t     https://BitOS.sh')}`))
  log(colors.heading.success.bold(`\n${isSmall ? '' : '\t '}  BitOS kernel v${rootPkgJson.version}   `))
  log(colors.warning(`${isSmall ? '' : '\t '}⚠         BETA         ⚠\n`))

  await printSystemInfo()

  if (!localStorage.getItem('BitOS_first_boot_complete')) {
  ...
  log(colors.danger(`\n${t('typeVerb', 'Type')} ${colors.bold.underline(Terminal.createSpecialLink('BitOS:execute:help', 'help'))} ${t('kernel:bootIntro.help', 'for help')}`))
  log(colors.gray(`${t('typeVerb', 'Type')} ${colors.bold.underline(Terminal.createSpecialLink('BitOS:execute:docs', 'docs'))} ${t('kernel:bootIntro.docs', 'to open the documentation')}`))
  log(colors.info(`${t('typeVerb', 'Type')} ${colors.bold.underline(Terminal.createSpecialLink('BitOS:execute:desktop', 'desktop'))} ${t('kernel:bootIntro.desktop', 'to launch the desktop')}`))
  // log(colors.primary(`${t('typeVerb', 'Type')} ${colors.bold.underline('wallet connect')} ${t('kernel:bootIntro.wallet', 'to connect your wallet')}`))
  log(colors.success(`${t('typeVerb', 'Type')} ${colors.bold.underline(Terminal.createSpecialLink('BitOS:execute:files /bin', 'files /bin'))} ${t('kernel:bootIntro.filesBin', 'to explore all executable commands')}`))
  ...
  log(colors.cyan(`${t('typeVerb', 'Type')} ${colors.bold.underline(Terminal.createSpecialLink('BitOS:execute:confetti', 'confetti'))} ${t('kernel:bootIntro.confetti', 'to fire the confetti gun 🎉')}`))
  ...
  if (bootArgs.get('source') !== 'pwa') {
    isSmall ? log('\n-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-') : log('\n-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-')
    log(colors.success(`${t('typeVerb', 'Type')} ${colors.bold.underline(Terminal.createSpecialLink('BitOS:execute:install', 'install'))} ${t('kernel:bootIntro.install', 'to install BitOS to your device')}`))
    isSmall ? log('-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-') : log('-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-')
  }

  log('\nhttps://docs.BitOS.sh')
  log('https://github.com/BitOS-org')
 ...
 * @param {?BitOSTerminal} options.terminal - The terminal to attach to, or undefined for global Terminal
 */
...
export async function dialog (options = {}) {
  return sweetalert.fire({
    ...
    customClass: { container: 'BitOS-kernel-dialog-container' },
    ...options
  })
}
...
export async function execute (cmd, options = {}) {
  ...
  if (!command?.run) {
    term.log(colors.danger(`Invalid command; try ${Terminal.createSpecialLink('BitOS:execute:help', colors.white('help'))} or ${Terminal.createSpecialLink(`BitOS:execute:3pm install ${exec}`, colors.blue(`3pm install ${exec}`))}`))
 ...
 * 
 * https://BitOS.sh/?initfsUrl=https://my.place.com/initfs.zip
 * 
 * Bare minimum, temporary, fs:
 * https://BitOS.sh/?mountableFilesystemConfig={ "/": { "fs": "InMemory" }, "/bin": { "fs": "InMemory" } }
 * 
 * @async
...
async function registerKernelBins () {
  ...
  kernelBins.docs = { description: t('kernel:bins.descriptions.docs', 'Open the documentation'), run: term => { modules.www.run(term, '--title "BitOS Documentation" --no-toolbar /docs') }}
  ...
  kernelBins.sh = { description: t('kernel:bins.descriptions.sh', 'Execute a BitOS script'), run: (term, context) => executeScript(context, { terminal: term }) }
  ...
  kernelBins.reboot = { description: t('kernel:bins.descriptions.reboot', 'Reload BitOS'), run: () => location.reload() }
  ...
  for (const [name, mod] of Object.entries(kernelBins)) {
    loadModule(mod, { name: `@BitOS-core/${name}` })
  }
}
...
async function registerBuiltinModules () {
  ...
  for (const mod of mods) {
    try {
      ...
      await loadModule(modBin, { name: `@BitOS-core/${mod}` })
    } catch (err) {
      console.error(err)
...
export async function loadModule (mod, options = {}) {
  ...
  const BitOSData = pkgJson?.BitOSData
  ...
  for (const pkg of packages) {
    tasks.push(new Promise(async (resolve, reject) => {
      try {
        ...
        const main = pkgJson.BitOSData.main || pkgJson.main || 'index.js'
        const type = pkgJson.BitOSData.type || 'es'
        const mainUrl = `${pkgJson.BitOSData.url}/${main}`
...
export async function showSplash (msg, options = {}) {
  ...
  title.id = 'BitOS-splash-title'
  title.innerHTML = options.title || 'BitOS'
  ...
  subtitle.id = 'BitOS-splash-subtitle'
  ...
  background.id = 'BitOS-splash-background'
...
export async function boot () {
  ...
  globalThis.addEventListener('beforeunload', async () => {
    ...
    document.querySelector('#BitOS-splash-icon').classList.add('rotating')
  })
  ...
  figlet.text('BitOS', { font: figletFontName }, async (err, logoFiglet) => {
    if (err) log(err)
    if (logoFiglet && globalThis.innerWidth >= 768) log(`\n${colors.green.bold(logoFiglet)}`)
    else log(`\n${colors.green.bold(`${isSmall ? '' : '\t\t'}🐉  BitOS 🐉`)}`)

    console.log(`%cBitOS %c${rootPkgJson.version}`, `
    ...
    console.log('%chttps://github.com/BitOS-org/kernel', 'font-size:14px;')
    ...
    for await (const [name, mod] of Object.entries(await fsModules({ BrowserFS, fs, execute, modules, t, utils }))) {
      loadModule(mod, { name: `@BitOS-fs/${name}` })
    }
    ...
    const BitOSCoreNamespaces = ['@BitOS-core', '@BitOS-fs']
    for (const mod of Object.values(modules)) {
      const [namespace, name] = mod.name.split('/')
      if (BitOSCoreNamespaces.includes(namespace)) modules[name] = mod
    }
    ...
    localStorage.setItem('BitOS_first_boot_complete', 'true')
    ...
    globalThis.BitOSBlinkyTitleInterval = setInterval(() => {
      document.title = document.title.includes('_') ? 'BitOS# ' : 'BitOS# _'
    }, 600)

    events.dispatch('BootComplete')
    console.timeEnd('BitOS:boot')
...
globalThis.addEventListener('beforeinstallprompt', e => {
  const installer = {
    name: '@BitOS-core/install',
    description: t('kernel:bins.descriptions.install', 'Install BitOS as a PWA'),
    run: async () => {
