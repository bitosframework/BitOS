 ...
 * BitOS Terminal
 * 
 * Unless you have a reason to directly create a new BitOSTerminal(),
 * you should probably use the static {@link BitOSTerminal.create} method
 * 
 * Prompt Format
 * 
 ...
 * @param {BitOSKernel=} [options.kernel=globalThis.Kernel] - The kernel for the terminal to attach to
...
export default class BitOSTerminal extends Terminal {
  cmd = ''
  cwd = '/'
  env = {}
  debug = false
  ...
  constructor (options = {}) {
    ...
    this.binSearchPath = options.binSearchPath || ['@BitOS-core', '@BitOS-fs', '@BitOS-apps', '@BitOS-utils']
  ...
  /**
   * Create a new BitOS Terminal instance with addons:
   * 
   * xterm-addon-fit, xterm-addon-web-links, xterm-addon-attach
   * 
   * @memberof BitOSTerminal
   * @param {Object} options - The options for the new terminal
   * @returns {BitOSTerminal}
   */
  static create (options = {}) {
    const term = new BitOSTerminal({ ...defaults, ...options })
 ...
 */
  specialLinkHandler (event, text, range, term) {
    ...
    if (parts[0] !== 'BitOS') return window.open(text)
