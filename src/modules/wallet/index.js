...
export const name = '@BitOS-core/wallet'
...
export const walletTypes = [
  onboardInjected(),
  onboardTrezor({
    appUrl: Kernel.get('wallet', 'trezor-appUrl') || 'https://BitOS.sh',
    email: Kernel.get('wallet', 'trezor-email') || 'nobody@BitOS.dev'
  })
]
    ...
    desktop: {
      ...
      transactionHandler: transaction => {
        ...
        if (transaction.eventCode === 'txPool') {
          return {
            type: 'success',
            message: t('app:wallet.txInMempool', 'Your transaction from BitOS is in the mempool'),
          }
        }
      },
...
export async function connect () {
  ...
  walletsSubscription.subscribe(wallets => {
    ...
    globalThis.localStorage.setItem('BitOS.wallet.wallets', JSON.stringify(connectedWallets))
  })
