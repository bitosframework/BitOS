...
async function loadFolder (browser, url) {
  ...
  const explorer = browser.querySelector('.BitOS-files-explorer')
  const addressBar = browser.querySelector('.BitOS-files-explorer-toolbar-addressbar-input')
  ...
  try {
    ...
    files.sort().forEach(file => {
      ...
      entry.classList.add('BitOS-files-explorer-entry')
      ...
      entry.addEventListener('contextmenu', e => {
        e.preventDefault()
        e.stopPropagation()
        ctxmenu.show(entryContextMenu, e.target.closest('.BitOS-files-explorer-entry').querySelector('mwc-icon'))
      })
...
export async function run (terminal = globalThis.Terminal, url) {
  ...
  browser.classList.add('BitOS-files-browser')
  toolbar.classList.add('BitOS-files-explorer-toolbar')
  addressBar.classList.add('BitOS-files-explorer-toolbar-addressbar-input')
  buttonWrapper.classList.add('BitOS-files-explorer-toolbar-button-wrapper')
  explorer.classList.add('BitOS-files-explorer')
  ...
  appWindow = kernel.windows.create({
    mount: browser,
    title: url,
    class: ['BitOS-window', 'BitOS-files-explorer-window'],
    height: '60%',
    width: '60%',
    x: 'center',
    y: 'center'
