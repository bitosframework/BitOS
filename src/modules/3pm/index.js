...
export const description = 'BitOS Package Manager'
export const help = `
  ${colors.blue('3pm')}: ${colors.green('The BitOS package manager')}
 ...
 * @property {string} pkgJson.BitOSData - Data specific to BitOS
 */
...
export async function install (url, args = { warn: true }) {
  console.log({ url })
  const warned = localStorage.getItem('BitOS_3pm_install_warning_hidden')
  if (!warned && args.warn) {
    localStorage.setItem('BitOS_3pm_install_warning_hidden', true)
  ...
  pkgJson.BitOSData = { pkgFolder, url, main, type }
  ...
  await mod.BitOSInstall?.(modInfo)
 ...
 * Run the 3pm CLI
 * @async
 * @param {BitOSTerminal} term - The terminal to attach to
